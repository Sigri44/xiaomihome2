...
def execute_action(message):
	...
	if "bg" in message['command']:
		ltype=LightType.Ambient
	else:
		ltype=LightType.Main
	if command_list[0] == 'turn':
		if command_list[1] == 'on':
			bulb.turn_on(light_type=ltype)
		else:
			bulb.turn_off(light_type=ltype)
	elif command_list[0] == 'stop':
		bulb.stop_flow(light_type=ltype)
	elif command_list[0] == 'rgb':
		red, green, blue = utils.hex_color_to_rgb(message['option'])
		bulb.set_rgb(red, green, blue, light_type=ltype)
	elif command_list[0] == 'toggle':
		bulb.toggle(light_type=ltype)
	elif command_list[0] == 'brightness':
		bulb.set_brightness(int(message['option']),light_type=ltype)
		...
		for transition in translist:
			...
			bulb.start_flow(flow,light_type=ltype)
		...
		bulb.set_hsv(int(hsv_option[0]), int(hsv_option[1]),light_type=ltype)
