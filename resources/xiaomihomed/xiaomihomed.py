...
def listen():
	...
	threading.Thread( target=xiaomiconnector, args=('aquara',)).start()
	...
	while 1:
		try:
			...
			if not JEEDOM_SOCKET_MESSAGE.empty():
				...
				message = JEEDOM_SOCKET_MESSAGE.get()
				...
				if message['cmd'] == 'send':
					logging.debug('SOCKET-READ------Executing action on : '+str(message['model']))
				...
				if message['cmd'] == 'read':
					logging.debug('SOCKET-READ------Executing read on : '+str(message['model']))
					if message['type'] == 'aquara':
						devices.aquara.execute_action(message)
				if message['cmd'] == 'refresh':
					logging.debug('SOCKET-READ------Refreshing : '+str(message['model']))
					if message['type'] == 'aquara':
				...
				if message['cmd'] == 'scanyeelight':
					logging.debug('SOCKET-READ------Scanning yeelight')
					devices.yeehome.scan(2)
				if message['cmd'] == 'discover':
					logging.debug('SOCKET-READ------Discovering : '+str(message['model']))
