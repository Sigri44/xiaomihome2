...
foreach ($eqLogics as $eqLogic) {
	...
	if ($eqLogic->getConfiguration('type') == 'aquara' && $eqLogic->getConfiguration('model') != 'gateway') {
		echo '<td><center><span class="label label-danger exclusion" style="font-size : 1em; cursor : pointer;" data-id="' . $eqLogic->getConfiguration('sid') . '" data-gateway="' . $eqLogic->getConfiguration('gateway') . '" style="font-size : 1em;"><i class="fas fa-times"></i></span></center></td></tr>';
	} else {
		echo '<td></td></tr>';
	}
}
